import { Controller, Post, Body, Param, UseGuards, Patch, Req } from '@nestjs/common';
...
export class ProtocolController {
    ...
    @Patch(':id/referee')
    @ApiOperation({ summary: 'Assign referee to match (Admin only)' })
    @ApiResponse({ status: 200, description: 'Referee assigned successfully' })
    @ApiResponse({ status: 404, description: 'Match or referee not found' })
    async assignReferee(
        @Param('id') id: string,
        @Body('refereeId') refereeId: string,
        @Req() req: any,
    ) {
        return this.protocolService.assignReferee(id, refereeId, req.user.id);
    }
}